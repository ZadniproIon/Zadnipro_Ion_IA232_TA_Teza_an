import os
import hashlib
import xml.etree.ElementTree as ET
import difflib
from sklearn.feature_extraction.text import TfidfVectorizer
from sklearn.metrics.pairwise import cosine_similarity

# ––––– utilitare –––––

def parse_metadata(file_path: str) -> tuple[str, str]:
    """Extrage titlul și autorii din fișierul text: primul rând non-vid este titlul,
    următorul rând care începe cu 'By ' conține autorii."""
    title = ''
    authors = ''
    with open(file_path, 'r', encoding='utf-8') as f:
        for line in f:
            line = line.strip()
            if not line:
                continue
            if not title:
                title = line
                continue
            if line.lower().startswith('by '):
                authors = line[3:].strip()
                break
    return title, authors


def md5_hash(text: str) -> str:
    """Returnează MD5 hex digest pentru un text."""
    return hashlib.md5(text.encode('utf-8')).hexdigest()


def compute_similarity(text1: str, text2: str) -> float:
    """Cosine similarity TF-IDF între două documente."""
    vecs = TfidfVectorizer().fit_transform([text1, text2])
    return float(cosine_similarity(vecs[0], vecs[1])[0, 0])


def find_matches_seq(susp: str, src: str, min_len: int) -> list[tuple[int,int,int]]:
    """Folosind difflib.SequenceMatcher, returnează toate blocurile de lungime >= min_len.
    Fiecare tuplu este (susp_offset, length, src_offset)."""
    matcher = difflib.SequenceMatcher(None, susp, src)
    matches = []
    for block in matcher.get_matching_blocks():
        i, j, n = block
        if n >= min_len:
            matches.append((i, n, j))
    return matches


def write_results(
    matches: list[tuple[int,int,int]],
    susp_name: str,
    src_name: str,
    susp_path: str,
    src_path: str,
    out_path: str,
    authors: str,
    title: str,
    lang: str,
    sim_score: float,
    severity: str,
    algorithm: str
):
    root = ET.Element('document', reference=susp_name)

    # metadate despre document
    ET.SubElement(root, 'feature', {
        'name':       'about',
        'authors':    authors,
        'title':      title,
        'lang':       lang,
        'similarity': f"{sim_score:.4f}",
        'severity':   severity,
        'algorithm':  algorithm
    })

    # hash MD5 al documentului suspicios
    with open(susp_path, 'r', encoding='utf-8') as f:
        doc_text = f.read()
    ET.SubElement(root, 'feature', {
        'name':  'md5Hash',
        'value': md5_hash(doc_text),
    })

    # segmente detectate
    for idx, (this_off, length, src_off) in enumerate(matches):
        ET.SubElement(root, 'feature', {
            'name':             'detected-plagiarism',
            'type':             'fuzzy-match',
            'this_language':    lang,
            'this_offset':      str(this_off),
            'this_length':      str(length),
            'source_reference': src_name,
            'source_offset':    str(src_off),
            'source_length':    str(length),
        })

    tree = ET.ElementTree(root)
    os.makedirs(os.path.dirname(out_path), exist_ok=True)
    tree.write(out_path, encoding='utf-8', xml_declaration=True)


def main():
    base_dir    = '00_spot_check'
    pairs_file  = os.path.join(base_dir, 'pairs')
    result_dir  = 'results-SequenceMatcher'
    os.makedirs(result_dir, exist_ok=True)

    # setări
    min_len   = 50
    algorithm = 'SequenceMatcher'
    lang      = 'en'

    # procesăm fiecare pereche
    with open(pairs_file, 'r', encoding='utf-8') as pf:
        for line in pf:
            line = line.strip()
            if not line or line.startswith('#'):
                continue
            susp_name, src_name = line.split()
            susp_path = os.path.join(base_dir, 'susp', susp_name)
            src_path  = os.path.join(base_dir, 'src', src_name)
            out_path  = os.path.join(result_dir, f"{susp_name[:-4]}-{src_name[:-4]}.xml")

            # afișăm progresul
            print(f"Processing {susp_name} vs {src_name}...")

            # metadate
            title, authors = parse_metadata(susp_path)

            # citim textele
            with open(src_path, 'r', encoding='utf-8') as sf:
                src_text = sf.read()
            with open(susp_path, 'r', encoding='utf-8') as tf:
                susp_text = tf.read()

            # detectăm și calculăm scorul global
            matches   = find_matches_seq(susp_text, src_text, min_len)
            sim_score = compute_similarity(susp_text, src_text)

            # severitate pe baza scorului
            if sim_score >= 0.99:
                severity = 'high'
            elif sim_score >= 0.95:
                severity = 'medium'
            else:
                severity = 'low'

            # scriem rezultate
            write_results(
                matches,
                susp_name, src_name,
                susp_path, src_path,
                out_path,
                authors, title,
                lang, sim_score,
                severity, algorithm
            )

            print(f" -> Done: {len(matches)} segments, sim={sim_score:.4f}\n")

if __name__ == '__main__':
    main()
